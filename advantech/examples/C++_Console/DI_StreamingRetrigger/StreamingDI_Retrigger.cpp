...
void BDAQCALL OnDataReadyEvent(void * sender, BfdDiEventArgs * args, void *userParam)
{
   ...
   do
   {
      ...
      printf("\nGetDataCount[ %d ]\n", getDataCount);
      ret = bfdDICtrl->GetData(getDataCount, dataBuf, 0, &returnedCount);
...
{
   ErrorCode        ret = Success;
   // Step 1: Create a 'Buffered DI Ctrl' for Streaming DI function.
   ...
   bfdDiCtrl->addCacheOverflowHandler(OnCacheOverflowEvent, NULL);
   bfdDiCtrl->addStoppedHandler(OnStoppedEvent, NULL);
	...
	do
	{
      ...
      ret = bfdDiCtrl->setSelectedDevice(devInfo);
		CHK_RESULT(ret);
      ret = bfdDiCtrl->LoadProfile(profilePath);
      CHK_RESULT(ret);
      ...
      CHK_RESULT(ret);
		...
		// for trigger0
      bool  retriggerable = bfdDiCtrl->getFeatures()->getDiRetriggerable();
