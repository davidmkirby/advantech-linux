...
int main(int argc, char* argv[])
{
   ...
   Record*   record      = NULL;
   int32     trgCount    = 0;
   CntrFeatures* cntrFeatures = NULL;
   ...
   do
   {
      ...
      cntrFeatures = BufferedEventCounterCtrl_getFeatures(bfdEventCounterCtrl);
      trgCount = CntrFeatures_getTriggerCount(cntrFeatures);
      if (trgCount)
      {
      ...
      CHK_RESULT(ret);
	   ...
	   ret = BufferedEventCounterCtrl_setEnabled(bfdEventCounterCtrl, FALSE);
	   CHK_RESULT(ret);
   }while(FALSE);
