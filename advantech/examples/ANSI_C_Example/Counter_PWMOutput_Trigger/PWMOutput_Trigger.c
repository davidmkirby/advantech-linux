...
*
* Description:
*    This example demonstrates how to use PWM Output function,also,users can enable trigger.
*
...
*    6  Optionally, the user can open the trigger.
*
...
//-----------------------------------------------------------------------------------
#define     deviceDescription  L"AIIS-1882,BID#0"
...
int main(int argc, char* argv[])
{
   ...
   do 
   {
      ...
      for (index = channelStart; index < channelStart + channelCount; index++)
      {
         ...
         ret = PoChannel_setTriggerSource(poChannel,SigExtDigTrigger0 + (index % channelCountMax));
         CHK_RESULT(ret);

         //if you want to disable trigger ,set like this
         //ret = PoChannel_setTriggerSource(poChannel,SignalNone);
         //CHK_RESULT(ret);


		 //Optionally,if you want to enable trigger,you need to set like this
		 ret = PoChannel_setTriggerEdge(poChannel,RisingEdge);
		 CHK_RESULT(ret);

		 //Optionally,if you want to enable trigger,you need to set like this
		 ret = PoChannel_setTriggerAction(poChannel,DelayToStart);
		 CHK_RESULT(ret);

         ret = PoChannel_setTriggerDelayCount(poChannel,1);// unit: second,delay 1 second
         CHK_RESULT(ret);
      }
      CHK_RESULT(ret);
