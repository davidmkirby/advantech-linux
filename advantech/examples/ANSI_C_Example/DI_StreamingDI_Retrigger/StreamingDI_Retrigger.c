...
*
* Windows Example:
*    StreamingDI_Retrigger.c
*
* Example Category:
*    DI
*
* Description:
*    This example demonstrates how to use Streaming DI with Retrigger function.
*
...
SignalDrop triggerSource = SigPFP0;
...
SignalDrop trigger1Source = SigPFP1;
...
{   
   ...
   do 
   {
      ...
      CHK_RESULT(ret);
      ret = ScanPort_setCycles(scanPort, cycles);
      CHK_RESULT(ret);
      ...
      diFeatures = BufferedDiCtrl_getFeatures(bfdDiCtrl);
      if (!DioFeatures_getDiRetriggerable(diFeatures))
      {
      ...
      retriggerable = DioFeatures_getDiRetriggerable(diFeatures);
...
void BDAQCALL OnDataReadyEvent(void * sender, BfdDiEventArgs * args, void *userParam)
{
   ...
   do
   {
      ...
      printf("\nGetDataCount[ %d ]\n", getDataCount);
      ret = BufferedDiCtrl_GetData(bfdDiCtrl, getDataCount, dataBuf, 0, &returnedCount, NULL, NULL, NULL);      
