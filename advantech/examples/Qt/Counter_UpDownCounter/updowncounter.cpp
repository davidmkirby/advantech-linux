...
void UpDownCounter::ConfiguratePanel() {
    disconnect(ui.cmbResetValue, SIGNAL(currentIndexChanged(int)), this, SLOT(CmbResetValueChanged()));
...
void UpDownCounter::TimerTicked() {
    ...
    CheckError(errorCode);
    ui.txtCounterValue->setText(QString("%1").arg(value));
}
...
void UpDownCounter::RadioButtonEnable()
{
    MathInterval resetRange;
    udCounterCtrl->getFeatures()->getUdValueResetTimes(resetRange);

    if(resetRange.Min == -1 && resetRange.Max == -1){
        ui.radDisable->setEnabled(false);
        ui.raInfinite->setEnabled(true);
        ui.raInfinite->setChecked(true);
        ui.radFinite->setEnabled(false);
    }

    if(resetRange.Min == -1 && resetRange.Max == 0){
