...
void ContinueCompare::ConfigureDevice() {
    ...
    errorCode = udCounterCtrl->LoadProfile(profile.c_str());
    CheckError(errorCode);
...
void ContinueCompare::ButtonStartClicked() {
	...
	if (dispersed == m_contCompTabSel) {
        ...
        udCounterCtrl->CompareAppendTable(udCounterCtrl->getChannelStart(), 3, ComTable);
	} else {
		CreateLineTable(0, param);
        udCounterCtrl->CompareAppendInterval(udCounterCtrl->getChannelStart(), param.firstValue, param.increment, param.count);
	}
    m_endCount = 0;
    m_matchCount = 0;
    m_tabIndex = 0;
...
void ContinueCompare::OnUdCounterEvent(void * sender, UdCntrEventArgs * args, void * userParam)
{
   ...
   {
      if(++uParam->m_tabIndex > 2)
          return;
      uParam->m_endCount++;
      if (uParam->m_contCompTabSel == dispersed)
      {
         uParam->CreateDisperseTable(uParam->m_tabIndex, comTable);
         udCounterCtrl->CompareAppendTable(channelStart, 3, comTable);
      }else{
         uParam->CreateLineTable(uParam->m_tabIndex, lineTabparam);
         udCounterCtrl->CompareAppendInterval(channelStart, lineTabparam.firstValue, lineTabparam.increment, lineTabparam.count);
      }
      matched = true;
