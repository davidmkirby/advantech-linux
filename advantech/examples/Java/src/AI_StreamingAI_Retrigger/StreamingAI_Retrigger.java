...
public class StreamingAI_Retrigger extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					StreamingAI_Retrigger frame = new StreamingAI_Retrigger();
	...
	public StreamingAI_Retrigger() {
		...
		wfAiCtrl.addStoppedListener(new StopEventListener());
	...
	public void Initialization(){
		...
		this.setTitle("Streaming AI with Retrigger - Run(" + configure.deviceName.substring(0, configure.deviceName.length() - 1) + ")");
	...
	protected void ConfigureDevice(){
		...
		try{
			...
			ErrorCode errorCode = wfAiCtrl.LoadProfile(configure.profileName);
			...
			wfAiCtrl.getRecord().setCycles(configure.cycles);
	...
	class StopEventListener implements BfdAiEventListener{
		@Override
		public void BfdAiEvent(Object arg0, BfdAiEventArgs arg1) {
	...
	class DataReadyEventListener implements BfdAiEventListener{
		public void BfdAiEvent(Object sender, BfdAiEventArgs args){
			...
			do {
				...
				if(Global.BioFaild(errorCode) && errorCode != ErrorCode.WarningRecordEnd){
					ShowMessage("Sorry, there're some errors occured, ErrorCode: " + errorCode.toString());
					return;
				}
				recordEndFlag = (errorCode == ErrorCode.WarningRecordEnd) ? true : false;
				graph.Chart(dataScaled, configure.channelCount, returnedCount.value / configure.channelCount, xInc, recordEndFlag);
