...
public class AsynchronousOneBufferedAI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AsynchronousOneBufferedAI frame = new AsynchronousOneBufferedAI();
	...
	public AsynchronousOneBufferedAI() {
		...
		label_YCoordinateMid.setBounds(0, 191, 46, 15);
		...
		lblColorOfChannels.setBounds(49, 386, 61, 38);
		...
		label_1.setBounds(49, 443, 46, 15);
		panel.add(label_1);
		...
		txtShift.setBounds(94, 440, 70, 21);
		panel.add(txtShift);
		...
		txtDiv.setBounds(359, 443, 70, 21);
		panel.add(txtDiv);
		
		lblDiv = new JLabel("ms");
		lblDiv.setBounds(434, 446, 22, 15);
		panel.add(lblDiv);
	...
	public void Initialization(){
		...
		this.setTitle("Asynchronous One Buffered AI - Run(" + configure.deviceName.substring(0, configure.deviceName.length() - 1) + ")");
	 ...
	 */
	 private void SetXCord(){
		 ...
		 double shiftMaxValue = round(graph.getXCordTimeDiv() * 10 + 
				 sliderShift.getValue(), 3, BigDecimal.ROUND_HALF_UP);
		 Global.GetXCordRange(X_rangeLabels, shiftMaxValue,
				 sliderShift.getValue(), timeUnit);
	...
	class StoppedEventListener implements BfdAiEventListener{
		public void BfdAiEvent(Object sender, BfdAiEventArgs args){
			 ...
			 */
			int remainingCount = args.Count;
			IntByRef returnedCount = new IntByRef();
			int getDataCount = 0;
			
			graph.Clear();
			do {
				getDataCount = Math.min(rawDataBufferLength, args.Count);
				ErrorCode errorCode = wfAiCtrl.GetData(rawDataBufferLength, dataScaled, 0, returnedCount, null, null, null);
				remainingCount -= returnedCount.value;
				if (Global.BioFaild(errorCode)) {
					ShowMessage("Sorry, there're some errors occred, ErrorCode: " + errorCode.toString());
					return;
				}				
				graph.Chart(dataScaled, configure.channelCount, returnedCount.value / configure.channelCount, 
