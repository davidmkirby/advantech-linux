...
public class ConfigureDialog extends JDialog {
	...
	{
		if(cmbTriggerSource.getItemCount() > 0){
		...
		// for trigger1 
		if(cmbTrigger1Source.getItemCount() > 0){
	...
	public void DeviceChanged(String deviceDescription)
	{
		try {
			...
			cmbTrigger1Source.removeAllItems();
			...
			cmbTrigger1Edge.removeAllItems();
	...
	public ConfigureDialog(AsynOneBufferedAI_TDtrtp parrent) {
		super(parrent);
		// Add window close action listener.
		addWindowListener(new WindowCloseActionListener());
	
		setTitle("Asynchronous One Buffered AI with Trigger Delay to Start and Delay to Stop - Configuration");
		setResizable(false);
		...
		lblProfile.setBounds(21, 61, 47, 15);
		contentPanel.add(lblProfile);
		
		txtProfilePath = new JFormattedTextField();
		txtProfilePath.setBounds(67, 58, 104, 20);
		contentPanel.add(txtProfilePath);
		
		btnBrowse = new JButton("Browse");
		btnBrowse.setBounds(181, 57, 93, 23);
    ...
    DeviceChanged(cmbDevice.getItemAt(0).toString());
//    ItemListener[] listener =  cmbDevice.getItemListeners();
//    ItemEvent e = new ItemEvent(cmbDevice, ItemEvent.SELECTED, cmbDevice.getItemAt(0), 0);
//    for(ItemListener I:listener)
//    {
//    	I.itemStateChanged(e);
//    }
	cmbDevice.setSelectedIndex(0);
		//cmbDevice.firePropertyChange("SelectedIndex", -1, 0);
	...
	class ButtonOKActionListener implements ActionListener{
		public void actionPerformed(ActionEvent arg0) {
			AsynOneBufferedAI_TDtrtp parrent = (AsynOneBufferedAI_TDtrtp)getParent();
