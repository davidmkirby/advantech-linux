...
public class StreamingAI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					StreamingAI frame = new StreamingAI();
	...
	public StreamingAI() {
		...
		panel.setFocusTraversalPolicy(new FocusTraversalOnArray(new Component[]{
				sliderDiv, btnConfig, btnStart, btnPause, btnStop}));
	...
	public void Initialization(){
		...
		this.setTitle("Streaming AI - Run(" + configure.deviceName.substring(0, configure.deviceName.length() - 1) + ")");
	...
	class DataReadyEventListener implements BfdAiEventListener{
		public void BfdAiEvent(Object sender, BfdAiEventArgs args){
			...
			do {
				...
				ErrorCode errorCode = wfAiCtrl.GetData(getDataCount, dataScaled, 0, returnedCount);
				...
				graph.Chart(dataScaled, configure.channelCount, returnedCount.value / configure.channelCount, xInc);
