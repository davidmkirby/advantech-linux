...
public class BufferedEventCounter extends JFrame implements ActionListener{
	...
	private BufferedEventCounterCtrl bfdEventCounterCtrl = new BufferedEventCounterCtrl();
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					BufferedEventCounter frame = new BufferedEventCounter();
	 ...
	 */
	public BufferedEventCounter() {
		setResizable(false);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 396, 526);
		...
		lblNewLabel_2.setBounds(25, 447, 287, 38);
		contentPane.add(lblNewLabel_2);
		contentPane.setFocusTraversalPolicy(new FocusTraversalOnArray(new Component[]{
				btnStart, btnStop, btnConfigure}));
	
		bfdEventCounterCtrl.addDataReadyListener(new DataReadyEventListener());
		bfdEventCounterCtrl.addStoppedListener(new StoppedEventListener());
	...
	public void Initialization() {	
		...
		this.setTitle("Buffered Event Counter - Run(" + configure.deviceName.substring(0, configure.deviceName.length() - 1) + ")");
	...
	private void ConfigureDevice(){
		try{
			//configure device
			bfdEventCounterCtrl.setSelectedDevice(new DeviceInformation(configure.deviceName));
			...
			if(configure.configureMode == 1){			
				...
				ErrorCode errorCode = bfdEventCounterCtrl.LoadProfile(profilePath);
				CheckError(errorCode);
				
				channelEnable = bfdEventCounterCtrl.getChannelMap();
				...
				bfdEventCounterCtrl.setChannelMap(configure.channelEnabled);
				bfdEventCounterCtrl.getRecord().setSectionLength(configure.sectionLength);
				bfdEventCounterCtrl.getRecord().setSectionCount(configure.sectionCount);
				bfdEventCounterCtrl.setSampleClkSrc(configure.sampleClkSrc);
							
				// Trigger Settings
				int trgCount = bfdEventCounterCtrl.getFeatures().getTriggerCount();
				if(trgCount != 0){
	...
	class ButtonStartActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			{		
				...
				try {
					bfdEventCounterCtrl.setEnabled(true);
				} catch (Exception ex) {
					ShowMessage("Sorry, there're some errors occured: " + ex.toString());
					return;
				}
	...
	class ButtonStopActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			try {
				bfdEventCounterCtrl.setEnabled(false);
			} catch (Exception ex) {
				ShowMessage("Sorry, there're some errors occured: " + ex.toString());
				return;
			}
	...
	class ButtonConfigureActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			if(bfdEventCounterCtrl.getEnabled()){
				try {
					bfdEventCounterCtrl.setEnabled(false);
				} catch (Exception ex) {
					ShowMessage("Sorry, there're some errors occured: " + ex.toString());
	...
	public class DataReadyEventListener implements BfdCntrEventListener {
		...
		public void BfdCntrEvent(Object arg0, BfdCntrEventArgs arg1) {
			...
			ErrorCode ret = bfdEventCounterCtrl.GetData(arg1.Channel, getDataCount, dataBuf, 0, null, null);
	...
	public class StoppedEventListener implements BfdCntrEventListener{
		...
		public void BfdCntrEvent(Object arg0, BfdCntrEventArgs arg1) {
			...
			if(bfdEventCounterCtrl.getState() == ControlState.Stopped){
				try {
					bfdEventCounterCtrl.setEnabled(false);
				} catch (Exception ex) {
					ShowMessage("Sorry, there're some errors occured: " + ex.toString());
