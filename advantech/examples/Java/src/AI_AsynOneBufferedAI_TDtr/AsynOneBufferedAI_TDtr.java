...
public class AsynOneBufferedAI_TDtr extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AsynOneBufferedAI_TDtr frame = new AsynOneBufferedAI_TDtr();
	...
	public AsynOneBufferedAI_TDtr() {
		...
		label_1.setBounds(49, 446, 46, 15);
		panel.add(label_1);
		...
		txtShift.setBounds(94, 443, 70, 21);
		panel.add(txtShift);
	...
	public void Initialization() {
		...
		this.setTitle("Asynchronous One Buffered AI with Trigger Delay to Start - Run(" + configure.deviceName.substring(0, configure.deviceName.length() - 1) + ")");
	...
	protected void ConfigureDevice() {
		try {
			...
			boolean isTrigger1Supported = (wfAiCtrl.getFeatures().getTriggerCount() > 1) ? true : false;
	...
	class StoppedEventListener implements BfdAiEventListener {
		public void BfdAiEvent(Object sender, BfdAiEventArgs args) {
			...
			do {
				getDataCount = Math.min(rawDataBufferLength, remainingCount);
				ErrorCode errorCode = wfAiCtrl.GetData(getDataCount, dataScaled, 0, returnedCount, null, null, null);
