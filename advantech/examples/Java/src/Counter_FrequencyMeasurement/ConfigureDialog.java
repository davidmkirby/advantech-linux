...
public class ConfigureDialog extends JDialog {
	 ...
	 */
	public ConfigureDialog(FrequencyMeasurement parrent) {
		super(parrent);
		// Add window close action listener.
		addWindowListener(new WindowCloseActionListener());
		
		setTitle("Frequency Measurement - Configuration");
		setResizable(false);
	...
	protected void Initialization() {
		...
		ArrayList<DeviceTreeNode> installedDevice = freqMeterCtrl.getSupportedDevices();
	...
	class ComboBoxDiveceItemListener implements ItemListener{
		public void itemStateChanged(ItemEvent e) {
			...
			if (e.getStateChange() == ItemEvent.SELECTED) {
				...
				try {
					FreqMeterCtrl freqMeterCtrl = new FreqMeterCtrl();
					freqMeterCtrl.setSelectedDevice(new DeviceInformation(selected));
					CntrFeatures feature = freqMeterCtrl.getFeatures();
					...
					for(int i = 0; i < channelCountMax; i++){
						...
						for(CounterCapability cap : capability){
							if(cap == CounterCapability.InstantFreqMeter){
								break;
							}
						}
	...
	class ButtonOKActionListener implements ActionListener{
		public void actionPerformed(ActionEvent arg0) {
			FrequencyMeasurement parrent = (FrequencyMeasurement)getParent();
