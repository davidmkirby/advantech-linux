		...
		public void UdCntrEvent(Object sender, UdCntrEventArgs args) {
			...
			if (evntEndId[udCounterCtrl.getChannelStart()] == args.Id) {
				...
				if (m_contCompTabSel == TabSel.dispersed) {
					CreateDisperseTable(m_tabIndex, comTable);
					udCounterCtrl.CompareAppendTable(udCounterCtrl.getChannelStart(), 3,  comTable);
				} else {
					CreateLineTable(m_tabIndex, lineTabparam);
					udCounterCtrl.CompareAppendInterval(udCounterCtrl.getChannelStart(), lineTabparam.firstValue,lineTabparam.increment,lineTabparam.count);
				}
				matched = true;
